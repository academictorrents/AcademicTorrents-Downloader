...
public final class TorrentFactory {
	...
	private static Torrent createTorrent(TorrentFile file, Properties properties)
			throws IOException {
		if (TorrentConfiguration.statePath == null) {
			throw new IllegalStateException("The folder to store "
					+ "configuration information has not been set yet");
		} else if (file == null) {
			throw new IllegalArgumentException("The torrent cannot be null");
		}
		synchronized (CREATED_TORRENTS) {
			Torrent torrent = (Torrent) CREATED_TORRENTS
					.get(file.getInfoHash());
			if (torrent == null) {
				File targetFile = file.getTargetFile();
				if (targetFile == null) {
					throw new IllegalArgumentException(
							"The target file or folder for "
									+ "this torrent has not been set yet");
				}
	 ...
	 */
	public static Torrent[] getSavedTorrents() throws IllegalStateException,
			IOException {
		if (TorrentConfiguration.statePath == null) {
			throw new IllegalStateException("The folder to store "
					+ "configuration information has not been set yet");
		}
		File[] files = TorrentConfiguration.statePath
				.listFiles(TORRENTS_FILTER);
