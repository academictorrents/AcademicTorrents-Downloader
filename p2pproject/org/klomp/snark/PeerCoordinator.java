...
import p2pp.DownloadProgressListener;
...
public class PeerCoordinator implements PeerListener
{
    ...
    // Some random wanted pieces. Is visible to subclasses [LIMA].
    protected final List<Integer> wantedPieces;
    ...
    // Monitor piece download progress [LIMA].
    protected DownloadProgressListener downloadProgress;
    ...
    /**
     * Used to set the progress listener [LIMA].
     */
    public void setDownloadProgressListener(DownloadProgressListener progress) {
    	this.downloadProgress = progress;
    	progress.setPeerCoordinator(this);
    }
    ...
    private static boolean peerIDInList (PeerID pid, List peers)
    {
        ...
        while (it.hasNext()) {
        	...
        	// A peerId is considered already in the list if contantains
        	// a peerId with the same ip and port [LIMA].
        	if(pid.equals(((Peer) it.next()).getPeerID())) {
                return true;
            }
        }
        return false;
    ...
    public int wantPiece (Peer peer, BitField havePieces)
    {
        ...
        synchronized (wantedPieces) {
            ...
            // Notify listener that a piece has been requested [LIMA].
        ...
        throws IOException
    {
        ...
        synchronized (wantedPieces) {
            ...
            try {
                if (storage.putPiece(piece, bs)) {
                    ...
                    // Notify progress listener about completed download 
                    // of a piece [LIMA].
                    if(downloadProgress != null)
                    	downloadProgress.pieceDownloaded(peer, piece);
                } else {
        ...
        if (completed()) {
            ...
            // Notify about completed download of all pieces [LIMA].
            if(downloadProgress != null)
