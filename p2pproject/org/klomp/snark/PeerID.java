...
{
    private byte[] id; //ID can change [LIMA].
    ...
    /**
     * If peer is created from binary encoded peer list obtained from the tracker, 
     * the id is not given. Therefore we can set it after the handshake is performed [LIMA].
     */
     ...
     * Hashcode is computed by using ip and port. [LIMA]
     */
    @Override
    public int hashCode ()
    {
        //return hash;
    	return (address.getHostAddress() + ":" + port).hashCode();
    }
    ...
    /**
     * Two PeerIDs are equal when they have the same (id,) address and port [LIMA].
     */
    @Override
    public boolean equals (Object o)
    {
        if (o instanceof PeerID) {
            ...
            boolean id = pid.id != null && this.id != null ? sameID(pid) : true;
            return port == pid.port && address.equals(pid.address) && id;
                //&& sameID(pid);
        } else {
            return false;
        }
    ...
    /**
     * Compares port, address (and id). Two peerId's are equal if they share
     * the same ip and port [LIMA].
     */
     ...
     * If the id is null return 'unknown' [LIMA].
     */
