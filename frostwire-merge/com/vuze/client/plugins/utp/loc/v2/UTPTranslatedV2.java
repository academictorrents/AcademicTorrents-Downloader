...
//			if (ASSERTS)_assert(delay < 0x2000000);
	...
	class UTPSocketImpl implements UTPSocket {
		...
		*/
		...
		boolean can_decay_win(UnsignedInteger msec) //const
		{
			return msec.minus( last_rwin_decay ) >= MAX_WINDOW_DECAY;
		}

		// If we can, decay max window, returns true if we actually did so
		void maybe_decay_win(long current_ms)
		{
			if (can_decay_win(new UnsignedInteger( current_ms))) {
				// TCP uses 0.5
				max_window = (int)(max_window * .5);
				last_rwin_decay.set( current_ms );
...
//				assert(-int64(dist_down) < INT_MAX / 4);
	...
	*/
								...
								int 	conn_id_send)
	{
		...
		conn.last_rwin_decay.set(conn.ctx.current_ms - MAX_WINDOW_DECAY);
